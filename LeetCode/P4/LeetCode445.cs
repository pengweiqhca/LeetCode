namespace LeetCode.P4
{
    public class LeetCode445
    {
        ...
        {
            Reverse(list, out var head);

            return head;
        }

        private static ListNode Reverse(ListNode list, out ListNode head)
        {
            if (list.next == null) return head = list;

            var next = Reverse(list.next, out head).next = list;
            next.next = null;
            return next;
        }
    }
